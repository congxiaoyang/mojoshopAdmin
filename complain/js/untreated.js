...
function pageLoad(curr) {
    $.ajax({
        type:"post",
        url:"http://192.168.1.111:8081/manager/complain/pending",
        dataType:"json",
        data:{
            "page":curr || 1,
            "rows":20    //显示的行数
        ...
        success:function (arr) {
            ...
            var tbody = $("#untreated_tbody");  //  tbody;
            var str;
            tbody.html("");
            if(status == 200){
                    ...
                    for(var i in data){
                        ...
                        str += '<tr data-id = "' + data[i].id+'">' +
                            ...
                            '<td> ' +
                                ...
                                '<input type="button" value="处理" class="goProcessed ml-10">' +
                            '</td> ' +
                            '</tr>';
                    }
...
// 点击 删除
$('#untreated_tbody').delegate(".deleteBtn","click",function(){
    var _this = $(this);
...
$('#untreated_tbody').delegate(".goProcessed","click",function(){
    var _this = $(this);
    var id = $(this).parents("tr").attr("data-id").trim();
    console.log(id);
    $('#btn-dialogBox').dialogBox({
        ...
        confirm: function(){
            ...
            $.ajax({
                type:"get",
                url:"http://192.168.1.111:8081/manager/complain/update_status",
                dataType:"json",
                data:{"id":id},
                success:function (arr) {
                    var backStatus = arr.status;
